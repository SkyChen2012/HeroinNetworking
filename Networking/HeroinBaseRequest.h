...
FOUNDATION_EXPORT NSString *const HeroinRequestValidationErrorDomain;

NS_ENUM(NSInteger) {
    HeroinRequestValidationErrorInvalidStatusCode = -8,
...
///  `HeroinResponseSerializerType`. Note this value can be nil if request failed.
///
...
///  If you use `HeroinResponseSerializerTypeJSON`, this is a convenience (and sematic) getter
...
///  Default is nil.
@property (nonatomic, weak, nullable) id<HeroinRequestDelegate> delegate;
...
@property (nonatomic, copy, nullable) HeroinRequestCompletionBlock successCompletionBlock;

///  The failure callback. Note if this value is not nil and `requestFailed` delegate method is
///  also implemented, both will be executed but delegate method is first called. This block
///  will be called on the main queue.
@property (nonatomic, copy, nullable) HeroinRequestCompletionBlock failureCompletionBlock;
...
///  The priority of the request. Effective only on iOS 8+. Default is `HeroinRequestPriorityDefault`.
@property (nonatomic) HeroinRequestPriority requestPriority;

///  Set completion callbacks
- (void)setCompletionBlockWithSuccess:(nullable HeroinRequestCompletionBlock)success
                              failure:(nullable HeroinRequestCompletionBlock)failure;
...
- (void)startWithCompletionBlockWithSuccess:(nullable HeroinRequestCompletionBlock)success
                                    failure:(nullable HeroinRequestCompletionBlock)failure;
...
///  Request serializer type.
- (HeroinRequestSerializerType)requestSerializerType;

///  Response serializer type. See also `responseObject`.
- (HeroinResponseSerializerType)responseSerializerType;
