...
void HeroinLog(NSString *format, ...) {
#ifndef DEBUG
    if (![HeroinNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
@implementation HeroinNetworkUtils
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        HeroinLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation HeroinBaseRequest (RequestAccessory)

- (void)toggleAccessoriesWillStartCallBack {
    for (id<HeroinRequestAccessory> accessory in self.requestAccessories) {
...
@implementation HeroinBatchRequest (RequestAccessory)
...
@implementation HeroinChainRequest (RequestAccessory)
